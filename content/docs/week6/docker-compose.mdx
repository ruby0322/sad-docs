...
> 現在 Docker Compose 已整合在 Docker CLI 中。若要將相同風格的設定部署到**多節點叢集**與滾動更新，可另學 [Docker Swarm 與 Stack](/docs/week6/docker-swarm)。
...
若要在**多台機器**上做叢集編排與滾動更新，可接續閱讀 [Docker Swarm 與 Stack](/docs/week6/docker-swarm)。接下來也建議配合 [SSH 與遠端部署](/docs/week6/ssh-deploy)，將 Compose 或映像部署到實際伺服器。
